...
## there are additional optional args to specify a DCOM ProgID for a compiled LabVIEW application, and a different username + password for remote host if that is required 
## options is a combination of the following flags (as per the #lvDCOMOptions enum in lvDCOMInterface.h)
...
#lvDCOMConfigure("ex2", "example", "$(TOP)/lvDCOMApp/src/examples/example_lvinput.xml", "ndxtestfaa", 3)

dbLoadRecords("$(TOP)/db/lvDCOM.db")
