...
<xsl:template name="extractRingText">
	<xsl:element name="items">
		...
		<xsl:choose>
		<xsl:when test="$n > 0">
			<xsl:comment>Ring Names extracted from LabVIEW, values autogenerated so may be wrong</xsl:comment>
			...
			<xsl:otherwise>
				<xsl:comment>Enum names cannot be extracted from LabVIEW - you need to add manually</xsl:comment>
				<xsl:element name="item">
